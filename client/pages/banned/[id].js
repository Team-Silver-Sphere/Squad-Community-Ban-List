    ...
    <>
      ...
      <Row className="mt-4">
        ...
        <Col className="text-right">
          <Link href={`/banned/${steamID}`} passHref>
            <Button color="primary" disabled={!(steamID && steamID.match(/^[0-9]{17}$/))}>
              Next
    ...
    <>
      <Row>
        <Col>
          ...
          <p>
            Hello<strong>{props.steamUser?.name ? ' ' + props.steamUser.name : ''}</strong>,
          </p>
          <p>
      ...
      <Row>
        <Col>
          ...
          <p>
            ...
            our <Link href="/faq">FAQ</Link> and then seek further help on our{' '}
...
export default function Search() {
  ...
  const { loading, error, data } = isValidSteam64ID
    ? useQuery(GET_STEAM_USER, { variables: { id } })
    : { loading: null, error: null, data: null };

  return (
    <>
      <NextSeo
        title="I'm banned, what now? | Squad Community Ban List"
        description="Get information on how to get unlisted from or unbanned by the Squad Community Ban List."
      />
      <RegularBanner />
